...
;
  ...
  if(ctx.getContext && (ctx = ctx.getContext("2d"))) {
    if(ctx.canvas) {
      ...
      if(ctx.fillText) {
        ctx.fillText("\ud83d\udca7", -4, 4);
        if(ctx.getImageData) {
          dont_have_clr_emoji = ctx.getImageData(0, 0, 1, 1).data[2] > 0; //RGBA, [2] is blue
        }
      }
    }
  }
  ...
  this.F(prefix,[1,[DRAW_VER(), 1, 0, dont_have_clr_emoji]]);
})();